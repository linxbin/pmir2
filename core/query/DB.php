...
class DB
{
	...
	{
		$action = current(explode(' ', $sql));

		if(in_array(strtolower($action), ['select','find','show','desc','use']))
		{
			return self::$Dbquery_read->query($sql,$queryMode);
		}else{
	...
	public function find()
	{
		...
		return self::$Dbquery_read->query($strSql,'Row');
	}
	...
	public function count()
	{
		...
		return (int)self::$Dbquery_read->query($strSql,'Row')['count'];
	}
	...
	public function select()
	{
		...
		return self::$Dbquery_read->query($strSql,'All');
	}
